...
LevelState::LevelState(Context* context) :
    GameState(context)
{
    Log::Write(-1," LevelState: Constructor \n");
...
{
    Log::Write(-1," LevelState: Destructor \n");
...
void LevelState::Enter(GameState* lastState)
{
    Log::Write(-1," LevelState::Enter\n");
    sub_state = -1;
...
void LevelState::Exit(GameState* nextState)
{
    Log::Write(-1," LevelState::Exit\n");
...
void LevelState::ChangeSubState(int newState)
{
    if (sub_state == newState)
        return;

    int oldState = sub_state;
    Log::Write(-1," LevelState: Switch State from " + String(oldState) + " to " + String(newState) + "\n");
...
void LevelState::CreateViewPort()
{
...
{
    Log::Write(-1," LevelState: CreateScene \n");
...
void LevelState::ShowMessage(const String& msg, bool show)
{
...
void LevelState::OnCharacterKilled(Character* killer, Character* dead)
{
    if (dead->side == 1)
    {
...
void LevelState::OnKeyDown(int key)
{
    if (key == KEY_Q)
    {
        if (RADIO::game_type == 1)
...
void LevelState::OnPlayerStatusUpdate(Player* player)
{
    if (!player)
        return;
...
void LevelState::OnSceneTimeScaleUpdated(Scene* scene, float newScale)
{
    if (gameScene != scene)
        return;
...
void LevelState::ApplyBGMScale(float scale)
{
...
String LevelState::GetDebugText()
{
